    ...
    internal static func retrieve (from row: FBSRow, at columnIndex: UInt32, type: FBSDatatype, statement: FrontbaseStatement) throws -> FrontbaseData {
        if fbsIsNull (row, columnIndex) {
            ...
            switch type {
                ...
                case FBS_Date:
                    throw FrontbaseError (problem: .error, reason: "Unexpected column type.", source: .capture())
                ...
                case FBS_Timestamp:
                    ...
                    if let timestamp = FrontbaseData.timestampFormatter.date (from: timestampString) {
                        ...
                        throw FrontbaseError (problem: .error, reason: "Unable to parse timestamp \(timestampString).", source: .capture())
                    }
                ...
                case FBS_CLOB:
                    ...
                    if let handle = fbsGetBlobHandle (row, columnIndex, &size) {
                        ...
                        throw FrontbaseError (problem: .error, reason: "No CLOB handle returned.", source: .capture())
                    }
                ...
                case FBS_BLOB:
                    ...
                    if let handle = fbsGetBlobHandle (row, columnIndex, &size) {
                        ...
                        throw FrontbaseError (problem: .error, reason: "No BLOB handle returned.", source: .capture())
                    }
