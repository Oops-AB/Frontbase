...
public enum FrontbaseDataType: SQLDataType {
    /// See `SQLDataType`.
    public static func dataType(appropriateFor type: Any.Type) -> FrontbaseDataType? {
        if let type = type as? FrontbaseDataTypeStaticRepresentable.Type {
            return type.frontbaseDataType
        }
        return nil
    }
