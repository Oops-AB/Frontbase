...
/// Encodes `Encodable` values to `FrontbaseData`.
///
///     let expr = try FrontbaseDataEncoder().encode("Hello")
///     print(expr) // .text("Hello")
///
/// Conform your types to `FrontbaseDataConvertible` to
/// customize how they are encoded.
public struct FrontbaseDataEncoder {
    /// Creates a new `FrontbaseDataEncoder`.
    public init() { }
    ...
    /// - returns: `FrontbaseData` representing the encoded data.
    public func encode (_ value: Encodable) throws -> FrontbaseData {
        if let value = value as? FrontbaseDataConvertible,
            let frontbaseData = value.frontbaseData {
            return frontbaseData
        } else {
            ...
            do {
                ...
                return .blob(FrontbaseBlob (data: json))
            }
        }
    }
    ...
    private struct _SingleValueEncodingContainer: SingleValueEncodingContainer {
        ...
        mutating func encode<T>(_ value: T) throws where T: Encodable {
            if let convertible = value as? FrontbaseDataConvertible {
                encoder.data = convertible.frontbaseData
                return
            }
            try value.encode(to: encoder)
        }
