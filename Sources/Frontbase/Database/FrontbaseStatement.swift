...
internal class FrontbaseStatement {
    ...
    internal func executeQuery() throws {
        ...
        guard connection.connection != nil else {
            throw FrontbaseError (problem: .error, reason: "Connection has been closed", source: .capture())
        }
        ...
        let resultSet: FBSResult? = withUnsafeMutablePointer (to: &errorMessage) { errorMessagePointer in
            return fbsExecuteSQL (connection.connection, sql + ";", connection.autoCommit, errorMessagePointer)
        }

        if let message = errorMessage {
            throw FrontbaseError (problem: .error, reason: String (cString: message), source: .capture())
        }

        self.resultSet = resultSet
    }

    internal func nextRow() throws -> [FrontbaseColumn: FrontbaseData]? {
        if let row = fbsFetchRow (resultSet) {
            let count = fbsGetColumnCount (resultSet)
            var result: [FrontbaseColumn: FrontbaseData] = [:]
            ...
            for columnIndex in 0 ..< count {
                ...
                result[column] = try FrontbaseData.retrieve (from: row, at: columnIndex, type: info.datatype, statement: self)
            }
